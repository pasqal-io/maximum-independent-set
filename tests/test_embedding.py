...
@pytest.mark.parametrize("graph, default_embedder_fails", [(simple_graph(), False), (dimacs_16nodes(), True)])
def test_easy_embedding(graph: nx.Graph, default_embedder_fails: bool) -> None:
    ...
    assert isinstance(solver._solver._embedder, DefaultEmbedder)
    ...
    opt_config = SolverConfig(preprocessor=None, backend = BackendConfig(), embedder=OptimizedEmbedder())
    opt_solver = MISSolver(instance, opt_config)
    assert isinstance(opt_solver._solver._embedder, OptimizedEmbedder)
    register = opt_solver.embedding()
    assert len(register.qubits) == len(graph)
    assert opt_solver._solver.backend.device().validate_register(register) is None
