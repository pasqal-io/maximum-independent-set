from tests.test_files.dimacs.graphs import load_dimacs
...
def test_hexagon():
    ...
    """
    dataset = load_dimacs("tests/test_files/dimacs/hexagon.txt")
...
def test_petersen():
    ...
    """
    dataset = load_dimacs("tests/test_files/dimacs/petersen.txt")
...
def test_a265032_1dc64():
    """
    Test loading the graph from a DIMACS file.
    """
    dataset = load_dimacs("tests/test_files/dimacs/a265032_1dc.64.txt")
...
def test_a2650321tc32():
    """
    Test loading the graph from a DIMACS file.
    """
    dataset = load_dimacs("tests/test_files/dimacs/a265032_1tc.32.txt")
    assert dataset.instance.graph.number_of_nodes() == 32
    assert dataset.instance.graph.number_of_edges() == 68
    assert len(dataset.solutions) > 0
    assert all(isinstance(solution, list) for solution in dataset.solutions)
    # graph has a maximum independent set of size 6
    for solution in dataset.solutions:
        assert Kernelization(dataset.instance.graph).is_independent(solution)
