...
def compute_min_max_u(register_coord, graph):
...
def simple_adiabatic_sequence(register, parameters) -> Sequence:
...
def simple_quantum_loop(parameters, register, run_qutip=True, run_emu_mps=False):
    ...
    seq = simple_adiabatic_sequence(register, parameters)
    # print(seq)
...
def q_solver(graph, coordinates_layout, num_sol, display_info=True):
    """MWIS quantum solver
    ...
    array_weights = np.array(
        [weights[node] for node in weights.keys()]
    )  # Create array with the weights of the graph
    ...
    interval_interaction = compute_min_max_u(
        coord, graph
    )  # Compute the min and max values for the amplitude and detuning
    ...
    spread_detuning = params["detuning_maximum"] * (
        1 - min_weight
    )  # Compute the detuning to used in DMM

    # array_weights = (array_weights - min_weight) / (1 - min_weight)

    weights_rev = 1 - array_weights  # Reverse the weigths for the DMM
    params["dmm_map"] = dict(
        zip(reg.qubit_ids, weights_rev)
    )  # Assigned the DMM map to each register
    ...
    solution = process_quantum_solution(
        count, reg, num_sol
    )  # Given the counts obtain a list of MWIS
